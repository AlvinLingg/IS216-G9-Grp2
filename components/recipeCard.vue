...
<template>
  <div
    ...
    v-for="recipe in props.recipes" @click="navigateTo('/recipes/' + recipe.id)">
    ...
    <div class="card-body pt-4 pb-2 px-2 gap-0">
      ...
      <template v-else>
        <p v-if="props.showIngredients" class="text-gray-500 text-sm">{{recipe.extendedIngredients.length}} Ingredients
        </p>
        <p v-if="props.showPreparationTime" class="text-gray-500 text-sm">{{recipe.readyInMinutes}} Minutes</p>
