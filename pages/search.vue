  ...
  <div>
    <div class="section p-5 max-w-6xl m-auto">
      <div class="flex flex-col gap-5">
        ...
        <div class="grid gap-6 mt-2 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 xs:grid-cols-1" ref="scrollComponent">
...
watch(searchQuery, async () => {
  searchResults.value = await searchRecipe(apiStore.apiIndex, searchQuery.value, 12);
})

watch(offset, async () => {
  let temp = await searchRecipe(apiStore.apiIndex, searchQuery.value, 12, offset.value);
  searchResults.value.results = searchResults.value.results.concat(temp.results);
})
