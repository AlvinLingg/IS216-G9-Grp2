                    ...
                    <template v-if="suggestedRecipes !== undefined">
                        <div v-for="recipe in Object.values(suggestedRecipes)">
                            <RecipeCard :showIngredients="true" :showPreparationTime="true" :recipe="recipe"
                                :isFridgeRecipe="true" @click="navigateTo(`/recipes/${recipe.id}`)" />
                        </div>
                    </template>
                </div>
            </div>
...
let suggestedRecipes = ref(await findByIngredients(apiStore.apiIndex, strIngredients.value, 8)); //remember to change to 12 later
